...
export default function DeviceView({
	...
	const previewPalette = device.palette_id
		? `&palette=${encodeURIComponent(device.palette_id)}&dither=${device.dithering_method ?? "floyd-steinberg"}`
		: `&grayscale=${grayscaleLevels}`;
	const heroSrc = isPlaylist
		? `/api/bitmap/${playlistScreens[0].screen || "simple-text"}.${previewExt}?width=${deviceWidth}&height=${deviceHeight}${previewPalette}`
		: isMixup
			? `/api/bitmap/mixup/${device.mixup_id}.${previewExt}?width=${deviceWidth}&height=${deviceHeight}${previewPalette}`
			: `/api/bitmap/${device?.screen || "simple-text"}.${previewExt}?width=${deviceWidth}&height=${deviceHeight}${previewPalette}`;
							...
							{playlistScreens.map((screen, i) => (
								...
								>
									<DeviceFrame size="sm" portrait={isPortrait} flat>
										<Image
											src={`/api/bitmap/${screen.screen || "simple-text"}.${previewExt}?width=${deviceWidth}&height=${deviceHeight}${previewPalette}`}
											alt={`Frame ${i + 1}`}
											fill
											className="absolute inset-0 h-full w-full object-cover"
											style={{ imageRendering: "pixelated" }}
